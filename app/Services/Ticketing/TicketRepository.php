...
class TicketRepository {
    ...
    public function paginateTickets($params)
    {
        ...
        if ($tagId === 'mine' && ! $assignee) {
            $assignee = Auth::user()->id;
        }

        // filter by tag
        if ($tagId && $tagId !== 'mine') {
            $query->filterByTag($tagId);
        }

        // filter by assignee
        if ($assignee) {
            $query->filterByAssignee($assignee);
        }

        // get only tickets that specified user has created
        if ($requester) {
            $query->filterByRequester($requester);
        }
        ...
        if ($orderBy = Arr::get($params, 'orderBy')) {
            ...
            $query->orderByRaw("FIELD({$prefix}tags.name, 'open', 'pending', 'closed', 'spam') asc, {$prefix}tickets.updated_at desc");
        }
